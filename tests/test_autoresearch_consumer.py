...
from hpa_mdo.autoresearch import AutoresearchPrimaryConfig, build_producer_cli_argv, load_primary_mass_run
...
def test_autoresearch_cli_prints_score_line(capsys, monkeypatch, tmp_path: Path) -> None:
    fake_run = consumer_impl.AutoresearchPrimaryRun(
        ...
        manifest={"producer_name": "hpa_mdo.dual_beam_joint_decision"},
        decision_interface={
            "schema_name": consumer_impl.EXPECTED_DECISION_SCHEMA_NAME,
            "schema_version": consumer_impl.EXPECTED_DECISION_SCHEMA_VERSION,
            "status": "complete",
        },
        decision_json_path=(tmp_path / "run" / "decision.json").resolve(),
