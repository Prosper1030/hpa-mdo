...
def test_goal_completion_audit_tracks_phase41_review_required_after_rankable_lambda() -> None:
        ...
        if item.key == "full_wing_global_buckling":
            item = SimpleNamespace(
                ...
                current_evidence=(
                    ...
                    "Phase30: closure status=reference_load_review_missing; "
                    "Phase41: braced subassembly status=braced_subassembly_solver_ran_mode_review_required; "
                    "max lambda=2.7857; Phase30 status=reference_load_review_missing. "
                ...
                next_action="Review mode shape and mesh sensitivity",
            )
        items.append(item)
    audit = build_structural_goal_completion_audit(
        SimpleNamespace(
    ...
    assert row.evidence_strength == "claim_boundary_plus_braced_route_review_required"
    assert row.completion_blocker == (
        "braced_subassembly_reference_load_mode_mesh_review_missing"
    )
...
def test_goal_completion_audit_tracks_existing_torque_observability_without_twist_closure() -> None:
    ...
    items = []
    for item in closure_index.items:
        if item.key == "torsion_twist_coupling":
            item = SimpleNamespace(
                key=item.key,
                evidence_artifacts=f"{item.evidence_artifacts}; Phase47",
                current_evidence=(
                    f"{item.current_evidence} Phase47: existing torsion/twist evidence "
                    "status=existing_torsion_twist_evidence_does_not_close_goal; "
                    "torque-observable rows=2; aeroelastic closure rows=0."
                ),
                remaining_blocker=item.remaining_blocker,
                next_action=item.next_action,
            )
    ...
    row = {row.key: row for row in audit.rows}["torsion_twist_coupling"]
    assert row.completion_status == "blocked"
    assert row.evidence_strength == (
        "existing_torque_observable_but_aeroelastic_twist_unclosed"
    )
    assert row.completion_blocker == "aeroelastic_twist_or_torque_couple_fem_missing"
