...
def test_near_wall_candidate_internalizes_span_caps_and_stitches_sharp_te_wake() -> None:
    ...
    assert summary["external_boundary_topology"]["status"] == "watertight"
    assert summary["external_boundary_topology"]["bad_edge_count"] == 0
    assert summary["sharp_te_boundary_stitch"]["status"] == "applied"
    assert summary["sharp_te_boundary_stitch"]["remapped_node_count"] > 0
    ...
    assert "near_wall_external_boundary_not_watertight" not in summary["blockers"]
...
def test_near_wall_candidate_probe_run_writes_artifacts(tmp_path: Path) -> None:
    ...
    assert summary["merged_volume"]["external_boundary_topology"]["status"] == "watertight"
    assert summary["merged_volume"]["external_boundary_topology"]["bad_edge_count"] == 0
    assert summary["merged_volume"]["sharp_te_boundary_stitch"]["status"] == "applied"
    ...
    assert "near_wall_external_boundary_not_watertight" not in summary["merged_volume"]["blockers"]
