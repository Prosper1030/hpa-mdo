...
def test_setup_gate_r20_star_tessellation_keeps_solver_blocked_until_mixed_mesh() -> None:
    module = _load_module()
    otherwise_ready_setup = {
        "physics_setup_id": "baseline_a_current_go_wall_resolved_rans_sa_alpha5",
        "solver": "INC_RANS",
    ...
    star = result["core_closure_topology"]["left_tip_star_tessellation"]
    assert star["schema_version"] == "wo006r20_left_tip_star_tessellation_probe.v1"
    assert star["matched_target_triangle_count"] == 12
...
def test_setup_gate_r22_global_star_split_refines_blocker_to_degenerate_cells() -> None:
    module = _load_module()
    otherwise_ready_setup = {
        "physics_setup_id": "baseline_a_current_go_wall_resolved_rans_sa_alpha5",
        "solver": "INC_RANS",
        ...
        core_closure_artifacts=[
            {
                ...
                "verdict": "global_star_split_basis_blocked",
    ...
    assert "near_wall_global_star_split_degenerate_cells" in result["blockers"]
    assert "near_wall_split_assembly_internal_nonconformal" not in result["blockers"]
    assert (
        result["core_closure_topology"]["recommended_repair"]
        == "reduce_degenerate_star_cells_before_mixed_mesh_writer"
    )
    star = result["core_closure_topology"]["global_star_split_basis"]
    assert star["schema_version"] == "wo006r22_global_star_split_basis_probe.v1"
    assert star["matched_target_triangle_count"] == 5594
    assert star["internal_split_leak_face_count"] == 0
    assert star["nonmanifold_split_face_count"] == 0
...
def test_setup_gate_r27_marker_repair_passes_handoff_gate() -> None:
    module = _load_module()
    otherwise_ready_setup = {
        "physics_setup_id": "baseline_a_current_go_wall_resolved_rans_sa_alpha5",
        "solver": "INC_RANS",
        ...
        core_closure_artifacts=[
            {
                ...
                "verdict": "mixed_su2_handoff_marker_quality_pass_solver_ladder_pending",
                "mixed_su2_handoff": {
                    "status": "mixed_su2_handoff_written",
                    "mesh_path": "/tmp/culled_global_star_mixed_handoff_r27_repaired.su2",
                    "node_count": 56873,
                    "volume_element_count": 332221,
                    "volume_element_type_counts": {"10": 332221},
    ...
    assert "near_wall_mixed_su2_boundary_marker_repair_not_applied" not in result["blockers"]
    assert "near_wall_mixed_su2_boundary_marker_blocked" not in result["blockers"]
    assert "direct_stageback_topology_plc_segment_facet" not in result["blockers"]
    handoff = result["core_closure_topology"]["mixed_su2_handoff"]
    assert handoff["marker_counts"] == {"wing_wall": 1992, "farfield": 2366}
    assert handoff["unmarked_boundary_face_count"] == 0
    assert (
        result["core_closure_topology"]["recommended_repair"]
        == "wire_culled_mixed_su2_handoff_into_solver_smoke_then_grid_ladder"
    )
