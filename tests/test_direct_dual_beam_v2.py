...
from scripts.direct_dual_beam_v2 import (  # noqa: E402
    BaselineDesign,
    CandidateArchive,
    DirectV2Candidate,
    ReducedMapConfig,
...
def _map_config() -> ReducedMapConfig:
    ...
    return ReducedMapConfig(
        ...
        delta_t_rear_outboard_max_m=0.0025,
...
def test_design_from_reduced_variables_preserves_main_taper_and_rear_outboard_step() -> None:
    baseline = _baseline_design()
    map_config = _map_config()

    main_t, main_r, rear_t, rear_r = design_from_reduced_variables(
        baseline=baseline,
        z=np.array([0.8, 0.75, 0.6, 1.0, 0.5]),
    ...
    assert np.isclose(rear_t[4] - rear_t[3], map_config.delta_t_rear_outboard_max_m)
    assert np.isclose(rear_t[5] - rear_t[4], 0.0)
    assert rear_t[4] > rear_t[3]
    assert np.isclose(rear_t[5], rear_t[4])
