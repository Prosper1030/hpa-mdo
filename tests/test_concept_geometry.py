...
def test_build_linear_wing_stations_interpolates_multipoint_twist_controls():
    concept = GeometryConcept(
        span_m=30.0,
        wing_area_m2=27.0,
        root_chord_m=1.30,
        ...
        twist_root_deg=2.0,
        twist_tip_deg=-3.0,
        twist_control_points=((0.0, 2.0), (0.35, 0.5), (0.70, -1.5), (1.0, -3.0)),
        tail_area_m2=4.0,
        cg_xc=0.30,
        segment_lengths_m=(3.0, 3.0, 3.0, 3.0, 3.0),
    )

    stations = build_linear_wing_stations(concept, stations_per_half=11)
    station_by_y = {round(station.y_m, 6): station for station in stations}

    assert station_by_y[0.0].twist_deg == pytest.approx(2.0)
    assert station_by_y[10.5].twist_deg == pytest.approx(-1.5)
    assert stations[-1].twist_deg == pytest.approx(-3.0)
...
def test_enumerate_geometry_concepts_can_size_tail_from_tail_volume():
    ...
    payload["mass_closure"]["enabled"] = False
    payload["geometry_family"]["planform_parameterization"] = "mean_chord"
    payload["geometry_family"]["tail_sizing_mode"] = "tail_volume"
    payload["geometry_family"]["tail_volume_coefficient_candidates"] = [0.45]
    payload["geometry_family"]["sampling"]["sample_count"] = 1
    payload["tail_model"]["tail_arm_to_mac"] = 4.0
    payload["geometry_family"]["primary_ranges"] = {
        "span_m": {"min": 35.0, "max": 35.0},
        "mean_chord_m": {"min": 0.90, "max": 0.90},
        "wing_loading_target_Npm2": {"min": 31.0, "max": 31.0},
    ...
    assert len(concepts) == 1
    concept = concepts[0]
    assert concept.tail_area_source == "derived_from_tail_volume_coefficient"
    assert concept.tail_volume_coefficient == pytest.approx(0.45)
    assert concept.tail_area_m2 == pytest.approx(0.45 * concept.wing_area_m2 / 4.0)


def test_enumerate_geometry_concepts_samples_spanload_bias_and_twist_controls():
    cfg_path = Path(__file__).resolve().parents[1] / "configs" / "birdman_upstream_concept_baseline.yaml"
    payload = load_concept_config(cfg_path).model_dump(mode="python")
    payload["mass_closure"]["enabled"] = False
    payload["geometry_family"]["planform_parameterization"] = "mean_chord"
    ...
    assert len(concepts) == 1
    concept = concepts[0]
    assert concept.spanload_bias == pytest.approx(0.08)
    assert [point[0] for point in concept.twist_control_points] == pytest.approx([0.0, 0.35, 0.70, 1.0])
    assert [point[1] for point in concept.twist_control_points] == pytest.approx([2.0, 0.75, -1.25, -2.75])
    stations = build_linear_wing_stations(concept, stations_per_half=8)
    assert stations[0].twist_deg == pytest.approx(2.0)
