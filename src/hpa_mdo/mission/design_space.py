...
def _coerce_outputs(value: Any) -> tuple[Path, bool, bool, bool]:
    ...
    return output_dir, write_full_results_csv, write_envelope_csv, write_markdown_report
...
def load_mission_design_space_spec(config_yaml: str | Path) -> MissionDesignSpaceSpec:
    ...
    output_dir, write_full, write_env_csv, write_report = _coerce_outputs(
        payload.get("outputs", {}),
    )
...
def _table_rows_from_iterable(
    ...
    lines = [
        f"### {title}",
        "",
        "| rank | speed | span | AR | CD0 | CLmax | margin(W) | cl_band | stall_band | power_passed |",
        "|---:|---:|---:|---:|---:|---:|---:|---|---|:--:|",
    ]
    for index, case in enumerate(rows[:limit], start=1):
        margin_text = (
            "n/a" if case.power_margin_crank_w is None else f"{case.power_margin_crank_w:.2f}"
        )
        lines.append(
            f"| {index} | {case.speed_mps:.2f} | {case.span_m:.1f} | {case.aspect_ratio:.1f} | {case.cd0_total:.3f} | {case.cl_max_effective:.2f} | {margin_text} | {case.cl_band} | {case.stall_band} | {case.power_passed} |"
        )
    return lines
...
def write_design_space_report(
    ...
    *,
    ...
    lines += [
        ...
        "",
        "- If a speed only passes at lower CD0, the design space is drag-sensitive at that speed.",
        "- If a design passes only when CLmax is high (e.g., 1.65), stall assumption is likely optimistic.",
        "- A positive margin case still near zero margin should not be treated as a primary candidate; keep a margin buffer.",
        "- CD0=0.020 passing only at low speeds is suitable for rescue/conservative modes rather than high-speed cruise.",
        "",
        "## Candidate Tables",
        "",
    ]
    ...
    conservative_clmax = [case for case in results if case.cl_max_effective <= 1.45]
    high_speed = [case for case in results if case.speed_mps >= 6.5]
    low_risk = [
        case
        for case in results
        ...
        and case.power_passed
    ]

    for section in [
        ("Robust candidates sorted by speed", robust_sorted_speed),
        ("Robust candidates grouped by CD0", robust_group_by_cd0),
        ("Conservative CLmax candidates (<=1.45)", conservative_clmax),
        ("High-speed candidates (speed >= 6.5 m/s)", high_speed),
        ("Low-risk candidates (healthy stall, margin >= robust threshold)", low_risk),
    ]:
        lines.extend(_table_rows_from_iterable(section[1], section[0]))
        lines.append("")
