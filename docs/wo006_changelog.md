# WO-006 CFD Development Log

2026-05 期間 WO-006 CFD 線的逐次探針紀錄，原本位於 `README.md` 前段（約 1,270 行），
2026-09 移出，讓 README 能作為專案入口而不是變更日誌。

**內容未經修改，逆時序保留。** 每一節對應一次 probe 或 gate 的實跑結果與工程判讀。

閱讀順序建議：這份文件是**過程紀錄**，不是結論。目前狀態請看
[`CURRENT_MAINLINE.md`](../CURRENT_MAINLINE.md) §0，以及
[hpa-mdo-framework 的 verification 摘要](https://github.com/Prosper1030/hpa-mdo-framework/blob/main/docs/verification.md)。
