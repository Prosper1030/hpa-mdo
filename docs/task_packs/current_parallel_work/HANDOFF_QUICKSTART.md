...
> 這一波最推薦直接用 [AGENT_LAUNCH_PLAN.md](AGENT_LAUNCH_PLAN.md) 的現成區塊，因為這一波重點是 ground-clearance recovery，不是再修 parser 或 solver。

## 最短版回答

可以。
你現在最簡單的做法，就是直接貼對應模板，然後把任務檔換成 `track_t_ground_clearance_recovery_outer_loop`。
...
你的目標是把目前 replay 裡明確暴露出的 `ground_clearance` 問題往前推，優先處理 outer-wing jig clearance recovery，而不是再修 parser / solver 或順手調 rib penalty / ranking 邏輯。

如果本地 repo context 不足，或工具 / solver / library 的行為可能已經變動，可以自行上網查，不要卡在舊文件裡。
上網查時優先看官方文件、solver manual、論文或其他第一手資料，並在回報中簡短說明查了什麼、如何影響你的判斷。

限制：
- 只能修改 `scripts/direct_dual_beam_inverse_design.py`, `scripts/direct_dual_beam_inverse_design_feasibility_sweep.py`, `tests/test_inverse_design.py`
...
- 一次派多個 agent：這一波先不要；先把 Track T 修好、驗過，再決定要不要重跑 Track R
