...
> 目標：在 Track X 重新挑出的 repaired AVL-first shortlist 上，重新回答 rib ranking 到底有沒有真實訊號。
...
請用 **Track X 新挑出的 repaired shortlist seeds**，重新做一輪小型 rib smoke。
...
- 優先用 Track X 明確推薦的 `2 到 4` 個 shortlist seeds
- 每個 seed 都要比：
  - `rib_zonewise=off`
  - `rib_zonewise=limited_zonewise`
...
1. 你實際跑了哪些命令
2. 這次實際採用了哪幾個 Track X shortlist seeds
3. 每個 seed 的：
   - `rib_zonewise=off` 結果
