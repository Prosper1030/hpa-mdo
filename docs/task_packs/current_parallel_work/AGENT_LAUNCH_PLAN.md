# AVL Repaired-Shortlist Agent Launch Plan

> 這份文件是給使用者直接複製貼上用的。  
> 目的不是重講整個 repo，而是讓你可以安全地啟動 **Phase 2 repaired AVL-first shortlist rebuild**，
> 然後再把 repaired shortlist 接回 rib smoke / tuning / finalist handoff。

## 1. 先講結論

現在建議的派工方式是：

- **Wave 18：先開 1 個 Track X agent**
- **等我驗證 Track X 之後**，再開 `Track R`
...
- 現在不需要再回頭修 ownership / load-state
- 正確順序是：**先重建 repaired shortlist，再用 repaired shortlist 重新跑 rib 煙霧測試，最後才決定 tuning 還是 finalist**
...
1. 先開 **Agent X**
2. 等我 review / verify Track X
3. 再開 **Agent R**
4. 等我 review / verify Track R
5. 如果結果是 suspicious 但有真 signal，開 **Agent M**
6. 如果結果已經 sane，直接開 **Agent N**

這樣可以先把 repaired AVL-first shortlist 重新建立，再用對的 seed 去做 rib smoke，不會再浪費時間在 drift 過或已過時的 baseline 上。
