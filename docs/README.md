...
## 2026-05-13 必讀：Bounded WO-006 Data Authority

Baseline A 目前不是 release/procurement truth；data authority 已恢復到只放行 bounded WO-006
SU2 aero calibration。任何 AI agent
...
screening only；WO-005 draft-only；WO-006 只能做 bounded aero calibration，不能當 release
truth、RFQ/procurement truth 或 final aircraft sign-off。
...
| [work_orders/QUEUE.md](work_orders/QUEUE.md) | Baseline A work-order queue；WO-006 may proceed only as bounded aero calibration using 98.5 kg and current pipeline span authority |
