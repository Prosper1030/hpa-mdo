...
> **維護者**：總工程師 + AI 架構師  
> **建立日期**：2026-04-09  
> **狀態**：Phase I 執行中（Milestone 2 Phase 1 完成）
...
```
  ...
  └────────┬────────────┘               └────────┬────────────┘
           ...
           │                                       │
           │ 3a. FSI one-way 接入 production      │
           │     pipeline (code 已存在)            │
           │ 3b. Multi-load-case production       │
           │     example + config                  │
           │ 3c. STEP export deformed shape       │
           │ 3d. Aero solver 模組化介面           │
           └──────────────┬───────────────────────┘
                          │
           ┌──────────────▼───────────────────────┐
           ...
           │                                       │
           │ 4a. 升力鋼索壓縮效應 (F10)           │
           │ 4b. Warping knockdown from rib geom  │
           │ 4c. 離散 OD post-processing (F11)    │
           │ 4d. 副梁重力力矩 (F12)               │
           │ 4e. 扭矩剪力 buckling (F8, if deferred)│
           └──────────────┬───────────────────────┘
                          │
           ┌──────────────▼───────────────────────┐
...
| Physics F6 (Thickness smoothness) | ❌ | M2 Phase 2 |
| Physics F8 (Torsion buckling) | ❌ | M2 Phase 2 conditional |
| Physics F10 (Wire compression) | ❌ | M4 |
| Physics F11 (Discrete OD) | ❌ | M4 |
| Physics F12 (Gravity torque) | ❌ | M4 |
| FSI one-way in production | ❌ | Code exists, not in pipeline — M3 |
| FSI two-way | ❌ | M5 |
| Multi-load-case production example | ❌ | Topology exists — M3 |
| STEP deformed shape | ❌ | M3 |
...
| Phase I — Milestone B | ✅ | Production → `method="auto"`, 21.57→13.91 kg (−35%) |
...
| 優先序 | 任務 | Phase | 預估工時 |
|--------|------|-------|----------|
| **1** | M2 Phase 2: F6 壁厚平滑 + F8 扭矩 buckling | I | 4-6 h |
| **2** | M3: FSI one-way 接入 production pipeline | I | 8-12 h |
| **3** | M3: Multi-load-case production example | I | 6-8 h |
| **4** | II-1: 擴充 materials.yaml（發泡材、Kevlar） | II | 4 h |
| **5** | M4: F10 wire compression + F11 discrete OD | I | 12-20 h |
