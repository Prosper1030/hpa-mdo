...
> **維護者**：總工程師 + AI 架構師  
> **建立日期**：2026-04-09  
> **狀態**：Phase I 收尾（Milestone 4 完成，僅餘 ANSYS 交叉驗證）
...
```
  ...
  └────────┬────────────┘               └────────┬────────────┘
           ...
           │ 4e. ANSYS cross-validation   ◀ NEXT  │
           └──────────────┬───────────────────────┘
                          │
           ┌──────────────▼───────────────────────┐
...
- [ ] ANSYS BDF + Workbench CSV 匯出與內部 FEM 結果交叉驗證 ← **最後一關**
...
Tests             = 145 passed, 1 skipped, 1 xpassed
```
...
|--------|------|-------|----------|------|
| **1** | M4-4e: ANSYS cross-validation（Phase I 最後一關） | I | 4-6 h | ❌ Prompt TBD |
...
| **6** | III-1: DOE 取樣器 + 批次 runner | III | 8-12 h | ❌ |

**關鍵路徑**：完成 ANSYS 交叉驗證 → Phase I 正式結束 → Phase II/III 可全面展開。
