...
> **文件性質**：長期藍圖 — 定義專案從求解器核心到全自動化設計 App 的五階段演進路線。這不是日常入口；如果你想知道「現在真正主線是什麼、先做什麼」，請先看 [CURRENT_MAINLINE.md](../CURRENT_MAINLINE.md)、[README.md](../README.md)、[docs/README.md](README.md)、[docs/NOW_NEXT_BLUEPRINT.md](NOW_NEXT_BLUEPRINT.md)。
> **維護者**：總工程師 + AI 架構師  
> **建立日期**：2026-04-09  
> **最後更新**：2026-04-17
> **狀態**：目前 canonical mainline 已收斂為 `cruise target -> inverse design -> jig shape -> realizable loaded shape -> CFRP / discrete layup`。Phase I-B M6-M9、M11 CLT、M13、M14、M-VSP Phase 2、M-HF4、M-HF5 已完成；generic VSP controls 已接進 AVL / ASWING 匯出鏈，wire pretension / tension gate / explicit truss 幾何已併入主線。經 2026-04-17 路線評估後：
...
```
  ...
  Phase I-B — 氣動彈性逆向設計主線（2026-04 新增）
             cruise target -> inverse design -> jig shape
             -> realizable loaded shape -> CFRP / discrete layup
           ...
           │     tail/fin sections in YAML/runtime│
           └──────────────┬───────────────────────┘
                          │
           ┌──────────────▼───────────────────────┐
           ...
           └──────────────┬───────────────────────┘
                          │
           ┌──────────────▼───────────────────────┐
           │ Milestone 13 — Controls Derivative    │
           │      Matrix Export（給控制組）         │
           │                                       │
           │ 13a. AVL 全套 .st stability derivative│
           │      解析器（Cxα, Cyβ, Clβ, Cmα,     │
           │      Cnβ, Clp, Cmq, Cnr, ...）       │
           │ 13b. control surface 導數抽取         │
           │      δE / δA / δR 對應 Cx/Cm/Cl/Cn   │
           │ 13c. 線性化 6-DOF 狀態空間 (A, B 矩陣)│
           │      以 trim 點為基準，輸出 JSON + CSV│
           │ 13d. 控制組交付介面文件               │
           │      docs/controls_interface_v1.md   │
           └──────────────┬───────────────────────┘
                          │
           ┌──────────────▼───────────────────────┐
           │ Milestone 14 — Mass / CG / Inertia    │
           │      Budget（系統整合）                │
           │                                       │
           │ 14a. hpa_mdo.mass.budget 模組         │
           │      結構 (from result) + 駕駛 +     │
           │      payload + 傳動 + 其他 systems   │
           │ 14b. config YAML: mass_budget schema │
           │      每項 {m_kg, x_m, y_m, z_m}      │
           │ 14c. CG + inertia tensor (I_xx,     │
           │      I_yy, I_zz, I_xz) 計算          │
           │ 14d. AVL .mass 檔輸出                │
           │      自動寫入 case 目錄供 M13 使用    │
           │ 14e. 質量報告 + 不確定度欄位           │
           │      mass_budget_report.md           │
           └──────────────────────────────────────┘
```

### 並行支線 — Mac mini 本機高保真驗證層（M-HF）

```
           ┌──────────────────────────────────────┐
           │ M-HF: Gmsh + CalculiX + ParaView     │
           │       （取代 Windows/ANSYS 最後一哩）   │
           │                                       │
           │ M-HF1 ✅ Gmsh STEP→.inp runner        │
           │ M-HF2 ✅ CalculiX 線性靜力 + frd 解析 │
           │ M-HF3 ✅ CalculiX BUCKLE 特徵值       │
           │ M-HF4 ✅ ParaView state generator     │
           │ M-HF5 ✅ Gmsh Physical Group → NSET   │
           └──────────────────────────────────────┘

詳細藍圖：docs/hi_fidelity_validation_stack.md
```

### 並行支線 — 通用 VSP 輸入（M-VSP）

```
           ┌──────────────────────────────────────┐
           │ M-VSP: 任何 .vsp3 都能直接進管線       │
           │                                       │
           │ Phase 1 ✅ span/root/tip/位置抽取     │
           │     scripts/analyze_vsp.py + CLI     │
           │ Phase 2 ✅ 翼型 AFILE / 控制面 /      │
           │     翼梁分段自動縮放 / 連續 dihedral  │
           │     tail/fin runtime / generic ctl   │
           └──────────────────────────────────────┘

Phase 2 prompt：docs/codex_prompts/M_VSP2_generic_intake_phase2.md
```
...
| discrete layup 正式 realization | ✅ | `--discrete-layup` 已接進主管線；連續 thickness 不再視為 final answer |
...
| wire pretension / tension gate | ✅ | pretension config + hard wire margin gate |
| explicit wire truss geometry | ✅ | nonlinear truss + 變形後 cable 幾何已對齊主線 |
...
這一章只保留高層摘要，方便把長期藍圖和近期執行藍圖切開。

- **目前正式工程主線**：`cruise target -> inverse design -> jig shape -> realizable loaded shape -> CFRP / discrete layup`。`producer / decision interface` 是外部整合邊界，不是主 physics 本體。
- **近期最高優先**：`P4#18 surrogate warm start`。
- **近期第二主線**：open-source aeroelastic spike，用來降低 ASWING 取得門檻對主線的阻塞。
- **資料化工作**：real vendor / hardware catalog 仍值得做，但屬於讓 ranking 更接近採購現實的後續層。
- **實驗型 sweep**：focused crossover sweep 只在 ranking 接近或 catalog/geometry 改變時再跑。
- **詳細近期任務與判斷規則**：請改看 [docs/NOW_NEXT_BLUEPRINT.md](NOW_NEXT_BLUEPRINT.md)。
...
| Phase I-B — Milestone 11 (CLT) | ✅ | Ply/CLT/PlyStack/snap/Tsai-Wu/step guard/formatter |
| Phase I-B — wire / truss integration | ✅ | pretension, tension gate, explicit truss, deformed cable geometry |
| Phase I-B — M-HF1/2/3/4/5 | ✅ | Gmsh runner + CalculiX static + BUCKLE + ParaView + named NSET |
| Phase I-B — M-VSP Phase 2 | ✅ | 通用 VSP intake、tail/fin runtime、generic controls |
...
| 優先序 | 任務 | Milestone | 狀態 |
|--------|------|-----------|------|
| **0** | **P4#18 surrogate warm start（GP / XGBoost optional dependency）** | Phase III bridge | ⏭️ **NEXT** |
| **1** | **M10a/b 路線釐清：ASWING 非阻塞化 + open-source aeroelastic spike（SHARPy Docker first）** | M10 | ⏭️ 研究 / 原型 |
| **2** | real vendor catalog / hardware catalog 資料化，讓 discrete OD / rigging ranking 更接近採購現實 | M9+ | ⏭️ 後續資料化 |
| **3** | focused crossover sweep（1.5→2.2）只在 vendor catalog 或新幾何改變 ranking 時再跑 | M9 | ⏭️ 有需要再做 |
| **4** | 可取得授權時再做 ASWING seed/run/report benchmark，不再卡主線 | M10a | ⏸️ Optional |

更完整的近期優先順序、開始條件、暫緩條件，請看 [docs/NOW_NEXT_BLUEPRINT.md](NOW_NEXT_BLUEPRINT.md)。
