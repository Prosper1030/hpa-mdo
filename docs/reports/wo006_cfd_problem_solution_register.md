...
| structured transition patch unit passes | Artificial unit writes `4` segmented collar bases, `4` pyramid collars, `40` transition prisms, and `24` tetra; transition rows grow `0.03 m -> 0.09 m` with max row growth ratio `3.0`; rim quad max edge ratio is about `500`; dual proxy reports pass and no hotspot edge-ratio blocker | A controlled growth topology can separate BL/collar vertices from larger core tetra vertices; this is qualitatively different from point-size hints | `write_phase3_structured_transition_patch_unit_su2()` writes `structured_transition_patch_unit/mesh.su2` and `.report.json` | Apply this multi-row prism transition pattern to the real-wing segmented collar before attempting another pps42/l3 core merge |
