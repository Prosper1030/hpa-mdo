...
- Engineering read: R27 cleared marker ownership, but R28 showed the repaired
  ...
  remaining named blocker is explicit conformal cap materialization for
  `tip_wall`, `te_wall`, and `closure_wall` before tetra-core merge.
...
| partial wing-only prism BL needs explicit caps | `wing_upper` / `wing_lower` only prism extrusion at `points_per_side=42`, 24 layers, growth `1.2` gives `124,416` prism cells with non-positive signed volume `0` and root-sidewall max aspect `966.04`, but `tip_wall`, `te_wall`, and `closure_wall` are intentionally absent from the handoff mesh | The primary wing BL itself can be made locally clean, but cap/closure surfaces must be materialized between the original wall and BL outer interface before the tetra core sees a watertight inner boundary | `write_phase3_partial_wing_prism_handoff_su2()` writes a caps-pending artifact and blocks `core_tetra_interface` with `blocked_until_caps_materialized` | Materialize conformal caps for `tip_wall=82`, `te_wall=28`, and `closure_wall=4` source faces, then retry tetra-core merge |
