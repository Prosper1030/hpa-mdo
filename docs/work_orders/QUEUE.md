...
- Baseline A status: team release can start coupon, 1 m wing-bay v2, C04 local FEM,
  carbon tube RFQ, control/propulsion interface work; not final aircraft sign-off.
...
| WO-002 | P0 | ready | Mass / CG / Margin Budget Ledger | chief engineering | Baseline A cannot scale by hand-calculated mass and CG |
| WO-003 | P0 | next after WO-002 | Design-Space Freeze Audit | chief engineering + aero/geometry | Confirm Baseline A is not obviously dominated before the team over-invests |
...
```text
/goal
In /Volumes/Samsung SSD/hpa-mdo, execute WO-002: Mass / CG / Margin Budget Ledger.
...
Task:
Create a central Baseline A mass / CG / margin ledger seeded from the current
Baseline A release and P1 closure artifacts. Each component row must include
component name, mass, x/y/z location, owner, source, confidence level
(estimate / quoted / measured / frozen), and flags for whether it affects CG,
drag, power, or structure. The system must compute gross mass, CG, static
margin, power margin, drag budget, and structural margin summary where current
data exists. Output mass_budget.csv, cg_summary.json, margin_budget.md, and a
readable markdown table or chart.

Boundary:
Do not change geometry, C04 architecture, tail design, spar specs, or Baseline A
reopen status. Estimate-level values must stay marked as estimates.

Required output:
- verdict: mass_cg_margin_ledger_ready / mass_cg_margin_ledger_incomplete
- changed files
- verification
- engineering caveats
- reviewer prompt
