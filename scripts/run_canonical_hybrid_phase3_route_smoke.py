...
def _surface_edge_topology(
    ...
    *,
    ...
    marker_combo_counts_by_kind: dict[str, dict[str, int]] = {}
    samples: list[dict[str, Any]] = []
    samples_by_kind: dict[str, list[dict[str, Any]]] = {}
    midpoints: list[tuple[float, float, float]] = []
    midpoints_by_kind: dict[str, list[tuple[float, float, float]]] = {}
    for edge, count in sorted(bad_edges.items()):
        key = str(count)
        ...
        kind_combo_counts = marker_combo_counts_by_kind.setdefault(kind, {})
        kind_combo_counts[combo] = kind_combo_counts.get(combo, 0) + 1
        for role in roles:
        ...
        midpoints_by_kind.setdefault(kind, []).append(midpoint)
        sample = {
        ...
        kind_samples = samples_by_kind.setdefault(kind, [])
        if len(kind_samples) < bad_edge_sample_limit:
            kind_samples.append(sample)
    ...
    return {
        ...
        "bad_edge_marker_combo_counts_by_kind": marker_combo_counts_by_kind,
        "bad_edge_midpoint_bounds": _bounds(midpoints) if midpoints else {},
        "bad_edge_midpoint_bounds_by_kind": {
            kind: _bounds(kind_midpoints)
            for kind, kind_midpoints in midpoints_by_kind.items()
        },
