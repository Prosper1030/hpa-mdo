#!/usr/bin/env python3
"""Audit axis-agnostic two-prism splits against the Baseline A BL/core interface.

R15 checked one common hexa-to-prism split direction and showed that it cannot
make the current R13 core interface conformal.  R16 broadens that hypothesis:
for each candidate near-wall hexa, try all three opposite-face split axes and
both diagonals, then choose the best local two-prism representation.

This is still a pre-handoff topology probe.  It does not create a mixed SU2
mesh, does not postprocess y+, and does not produce CFD coefficients.
"""
...
DEFAULT_OUTPUT_DIR = WO006_ROOT / "wo006r16_axis_agnostic_prism_split_probe"
...
def axis_agnostic_two_prism_patterns() -> tuple[
    tuple[str, tuple[tuple[int, int, int, int, int, int], ...]],
    ...,
]:
    patterns: list[tuple[str, tuple[tuple[int, int, int, int, int, int], ...]]] = []
    for axis_name, permutation in AXIS_PERMUTATIONS:
        for split_index, canonical in enumerate(CANONICAL_TWO_PRISM_SPLITS):
            patterns.append(
                (
                    f"{axis_name}_{split_index}",
                    tuple(
...
def audit_axis_agnostic_prism_split_compatibility(
    *,
    ...
    patterns = axis_agnostic_two_prism_patterns()
    ...
    status = (
        ...
        else "blocked_by_incompatible_axis_agnostic_prism_split"
    )
    ...
    blockers: list[str] = []
    if status != "pass":
        blockers.append("axis_agnostic_two_prism_split_does_not_match_core_interface")
    if unmatched_by_marker:
        blockers.append("shared_interface_tessellation_missing")

    return {
        "schema_version": "wo006r16_axis_agnostic_prism_split.v1",
        "status": status,
        "blockers": blockers,
        ...
        "engineering_read": (
            "The axis-agnostic per-cell prism split can reproduce every current "
            "core interface triangle, but this still needs a written mixed mesh, "
            "marker ownership, y+, and solver gates before CFD."
            if status == "pass"
            else "Trying all three two-prism split axes improves the handoff match, "
            "but it still cannot reproduce every current R13 core interface triangle. "
            "The remaining wake/edge/loop-cap triangles require shared interface "
            "tessellation or boundary-driven near-wall remeshing before SU2."
        ),
    }
...
def build_probe_summary(
    *,
    ...
    if audit.get("status") != "pass":
        blockers.append("axis_agnostic_prism_split_handoff_blocked")
    ...
    return {
        ...
        "verdict": (
            "axis_agnostic_prism_split_handoff_pass_handoff_still_missing"
            if audit.get("status") == "pass"
            else "axis_agnostic_prism_split_handoff_blocked"
        ),
...
def run_probe(
    *,
    ...
    audit = audit_axis_agnostic_prism_split_compatibility(
    ...
    (output_dir / "axis_agnostic_prism_split_report.md").write_text(
...
def render_report(summary: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# WO-006R16 Axis-Agnostic Prism Split Probe",
            "",
            ...
            f"- matched core triangles by marker: `{audit['matched_core_triangles_by_marker']}`",
...
def _split_boundary_triangle_keys_for_cell(
    ...
    *,
    split_pattern: Sequence[Sequence[int]],
    ...
    for face in _split_exterior_faces(hex_nodes, split_pattern=split_pattern):
        if len(face) != 3:
            continue
        face_nodes = set(face)
