#!/usr/bin/env python3
"""Reduced direct dual-beam optimizer V2.x using a smarter 6D production map."""
...
def build_reduced_map_config(
    *,
    ...
    """Build the smarter V2.x map constants from the baseline specimen."""

    n_seg = int(baseline.main_r_seg_m.size)
    if n_seg != 6 or baseline.rear_r_seg_m.size != 6:
        raise ValueError(
            "direct_dual_beam_v2x currently supports the Black Cat 004 six-segment reduced map only."
        )
    ...
    # Bias the rear outboard thickness knob toward the tip segment where the
    # local psi_u_all leverage is strongest, while keeping the seg5->6 step smooth.
    ...
    outboard_idx = np.where(rear_outboard_mask > 0.0)[0]
    delta_t_rear_outboard_max_m = float(
        max(
            min(
                step_margin_45,
    ...
    return ReducedMapConfig(
        ...
        main_tip_fill_upper=float(max(main_tip_fill_upper, 0.0)),
...
def decode_reduced_variables(
    *,
    ...
    """Decode the normalized [0, 1]^6 vector into physical reduced variables."""

    z_arr = np.asarray(z, dtype=float).reshape(6)
    ...
    main_tip_fill = float(z_arr[2]) * float(map_config.main_tip_fill_upper)
    rear_radius_scale = math.exp(float(z_arr[3]) * rear_radius_log_upper)
    rear_outboard_fraction = float(z_arr[4])
    wall_thickness_fraction = float(z_arr[5])

    return {
        "main_plateau_scale": float(main_plateau_scale),
        "main_taper_fill": float(main_taper_fill),
        "main_tip_fill": float(main_tip_fill),
...
def design_from_reduced_variables(
    *,
    ...
    """Map the six reduced V2.x variables to full segment design arrays."""
...
class ProductionSmoothEvaluator:
    ...
    def _key(self, z: np.ndarray) -> tuple[float, ...]:
        return tuple(np.round(np.asarray(z, dtype=float).reshape(6), 10))
    ...
    def evaluate(self, z: np.ndarray, *, source: str) -> DirectV2Candidate:
        ...
        try:
            ...
            candidate = DirectV2Candidate(
                ...
                main_tip_fill=float(vars_physical["main_tip_fill"]),
...
def run_direct_dual_beam_v2x(
    *,
    ...
    main_tip_fill_grid: Iterable[float],
    ...
    baseline = evaluator.evaluate(np.zeros(6, dtype=float), source="baseline")

    coarse_grid = list(
        product(
            _grid_fraction(main_plateau_grid),
            _grid_fraction(main_taper_fill_grid),
            _grid_fraction(main_tip_fill_grid),
...
def candidate_to_summary_dict(candidate: DirectV2Candidate) -> dict[str, object]:
    return {
        ...
        "reduced_variables": {
            ...
            "main_tip_fill": candidate.main_tip_fill,
...
def build_report_text(
    *,
    ...
    lines: list[str] = []
    lines.append("=" * 108)
    lines.append("Direct Dual-Beam V2.x Report (Smarter 6D Production Map)")
    lines.append("=" * 108)
    lines.append(f"Generated                     : {generated}")
    lines.append(f"Config                        : {config_path}")
    lines.append(f"Design report                 : {design_report}")
    lines.append(f"Cruise AoA                    : {cruise_aoa_deg:.3f} deg")
    lines.append("Loop                          : baseline specimen -> reduced 6D grid -> COBYLA continuation")
    lines.append("")
    ...
    lines.append(f"  main_tip_fill upper         : {map_config.main_tip_fill_upper:.4f}")
    ...
    lines.append(f"  main_tip_fill                : {selected.main_tip_fill:.6f}")
    ...
    *,
    ...
    return {
        ...
        "map_config": {
            ...
            "main_tip_fill_upper": map_config.main_tip_fill_upper,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run reduced direct dual-beam optimizer V2.x with the smarter 6D production map."
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(Path(__file__).resolve().parent.parent / "output" / "direct_dual_beam_v2x_baseline"),
        help="Directory for the V2.x report and JSON summary.",
    )
    parser.add_argument("--main-plateau-scale-upper", type=float, default=1.14)
    parser.add_argument("--main-taper-fill-upper", type=float, default=0.24)
    parser.add_argument("--main-tip-fill-upper", type=float, default=0.12)
    parser.add_argument("--rear-radius-scale-upper", type=float, default=1.08)
    parser.add_argument("--main-plateau-grid", default="0.0,0.33,0.67,1.0")
    parser.add_argument("--main-taper-fill-grid", default="0.0,0.17,0.33,0.67")
    parser.add_argument("--main-tip-fill-grid", default="0.0,0.17,0.42,0.67")
    parser.add_argument("--rear-radius-grid", default="0.0,0.25,0.50,0.75,1.0")
    parser.add_argument("--rear-outboard-grid", default="0.0,0.01,0.02,0.03,0.05")
    parser.add_argument("--wall-thickness-grid", default="0.0")
...
def main(argv: list[str] | None = None) -> int:
    ...
    map_config = build_reduced_map_config(
        ...
        main_tip_fill_upper=float(args.main_tip_fill_upper),
        rear_radius_scale_upper=float(args.rear_radius_scale_upper),
    )

    outcome = run_direct_dual_beam_v2x(
        cfg=cfg,
        aircraft=aircraft,
        materials_db=materials_db,
        optimizer=optimizer,
        ...
        main_tip_fill_grid=_parse_grid(args.main_tip_fill_grid),
    ...
    report_path = output_dir / "direct_dual_beam_v2x_report.txt"
    report_path.write_text(report_text, encoding="utf-8")

    json_path = output_dir / "direct_dual_beam_v2x_summary.json"
    ...
    selected = outcome.selected
    print("Direct dual-beam V2.x complete.")
