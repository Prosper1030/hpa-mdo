...
def _hard_gate_notes(
    *,
    ...
    notes: list[str] = []
    if float(metrics["worst_case_margin"]) < 0.0:
        notes.append(
            "stall margin "
            f"{float(metrics['worst_case_margin']):.3f} at {metrics['worst_case_label']}"
        )
    if float(metrics["candidate_thickness_ratio"]) < zone_min_tc_ratio:
        notes.append(
            f"t/c {float(metrics['candidate_thickness_ratio']):.3f} < {zone_min_tc_ratio:.3f}"
        )
    if float(metrics["spar_depth_ratio"]) < float(metrics["required_spar_depth_ratio"]):
        notes.append(
            "spar depth "
            f"{float(metrics['spar_depth_ratio']):.3f} < {float(metrics['required_spar_depth_ratio']):.3f}"
        )
    cm_violation = max(0.0, -float(result["mean_cm"]) - abs(float(cfg.cst_search.cm_hard_lower_bound)))
    if cm_violation > 0.0:
    ...
    *,
    ...
    if bool(cfg.cst_search.coarse_to_fine_enabled):
        ...
        for zone_name, coarse_candidates in coarse_candidates_by_zone.items():
            ...
            beam_count = min(max(1, int(cfg.cst_search.coarse_keep_top_k)), len(scored))
...
def _write_report(
    *,
    ...
    lines.extend(["", "## Smoke All-Zone Historical Baseline Comparison", ""])
    lines.extend(
        _markdown_table(
    ...
    lines.extend(["", "## Representative Historical Baseline Comparison", ""])
    lines.extend(
        _markdown_table(
