...
- `main_wing` / `tail_wing` / `fairing_solid` / `fairing_vented` 的 schema、family dispatch、route registry 已經存在；`main_wing` 已有 real ESP/VSP geometry、real Gmsh mesh handoff、real SU2 handoff、solver executed but not converged artifact、reference-geometry warn gate，以及 station-seam BRep / same-parameter feasibility evidence；目前主翼產品化 blocker 是 station PCurve / seam topology repair，不是 production CFD pass；`tail_wing` 已有 real geometry / surface / blocker probes，`fairing_solid` 已有 real VSP geometry smoke、bounded real mesh handoff probe、real SU2 handoff materialization probe 與 external reference override handoff，但都還不是正式可交付 CFD 路徑。
...
- 目前真正的 blocker 已經往後移：coarse bounded real mesh handoff 和 real SU2 handoff 都已經 materialize，`SU2_CFD` 也能執行並寫出 `history.csv`；但 12-iteration smoke 和 OpenVSP-reference 80-iteration follow-up 都是 `fail/not_comparable`，80-iteration run 已保留 `surface.csv` 與 `forces_breakdown.dat`，main-wing reference chord 已可用 OpenVSP/VSPAERO `cref` cross-check，reference area / moment origin 仍是比較性 blocker。後續 station-seam evidence 又把更早的幾何 blocker 定位到曲線 36 / 50：PCurves 存在，但 curve-3D-with-PCurve / same-parameter / vertex-tolerance checks 不一致，且 `BRepLib.SameParameter` tolerance sweep 不能修復。
- 結論：`esp_rebuilt` 現在是「provider runnable + route artifact exists, but not production CFD」。下一步不是再補 runtime 安裝，也不是宣稱 solver converged；優先要處理 station PCurve / seam topology，再回來做 panel-vs-SU2 force-breakdown、reference provenance 與有根據的 numerics campaign。
...
### 22. Write the main wing mesh-handoff smoke
...
### 23. Write the main wing SU2-handoff smoke
...
### 24. Write the tail wing ESP-rebuilt geometry smoke
...
### 25. Write the tail wing mesh-handoff smoke
...
### 26. Write the tail wing SU2-handoff smoke
...
### 27. Probe the real tail wing mesh handoff
...
### 28. Probe the real tail wing surface mesh
...
### 29. Probe naive tail wing solidification
...
### 30. Probe explicit tail wing volume routes
...
2. inspect or rebuild the main-wing station PCurves / seam topology around curves 36 and 50 before promoting a compound meshing policy or spending more solver budget
