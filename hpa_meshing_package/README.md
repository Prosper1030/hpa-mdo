...
### 32. Diagnose side-aware station-seam PCurve residuals

```bash
cd /Volumes/Samsung\ SSD/hpa-mdo/hpa_meshing_package
PYTHONPATH=src /Volumes/Samsung\ SSD/hpa-mdo/.venv/bin/python -m hpa_meshing.cli main-wing-station-seam-side-aware-pcurve-residual-diagnostic \
  --out .tmp/runs/main_wing_station_seam_side_aware_pcurve_residual_diagnostic
```

This report-only diagnostic samples the side-aware candidate station edges
against their PCurves on owner faces. The current committed result is
`side_aware_station_pcurve_residuals_below_tolerance_but_shape_analysis_flags_fail`:
12 edge-face pairs were sampled, max 3D-vs-PCurve residual is `0.0 m`, and old
fixture tags are not replayed. The route is still blocked because all 12
ShapeAnalysis / same-parameter / vertex-tolerance flags fail and the PCurves
are unbounded `Geom2d_Line` domains. This points to a bounded metadata repair
gate, not a solver-iteration or convergence gate.

### 33. Probe side-aware station-seam metadata repair

```bash
cd /Volumes/Samsung\ SSD/hpa-mdo/hpa_meshing_package
PYTHONPATH=src /Volumes/Samsung\ SSD/hpa-mdo/.venv/bin/python -m hpa_meshing.cli main-wing-station-seam-side-aware-metadata-repair-probe \
  --out .tmp/runs/main_wing_station_seam_side_aware_metadata_repair_probe
```

This report-only probe runs bounded `BRepLib.SameParameter` and
`ShapeFix_Edge` repair attempts on the side-aware candidate station edges. The
current committed result is `side_aware_station_metadata_repair_not_recovered`:
the residual context is preserved (`max_sample_distance_m=0.0`, 12
ShapeAnalysis flag failures), 5 SameParameter tolerances and 25 ShapeFix
operation/tolerance attempts were evaluated, and both have
`recovered_attempt_count=0`. This means the next repair target is a PCurve
rewrite or export-metadata builder; it is not a mesh-handoff or convergence
claim.

### 34. Probe side-aware station-seam PCurve metadata builder

```bash
cd /Volumes/Samsung\ SSD/hpa-mdo/hpa_meshing_package
PYTHONPATH=src /Volumes/Samsung\ SSD/hpa-mdo/.venv/bin/python -m hpa_meshing.cli main-wing-station-seam-side-aware-pcurve-metadata-builder-probe \
  --out .tmp/runs/main_wing_station_seam_side_aware_pcurve_metadata_builder_probe
```

This report-only probe tests explicit in-memory PCurve metadata construction
strategies on the side-aware candidate. The current committed result is
...
Four bounded-existing-PCurve strategies make 12 / 12 domains bounded, but still
leave 0 / 12 edge-face pairs passing same-parameter, curve-3D-with-PCurve, and
vertex-tolerance checks. This is partial metadata progress only; the follow-up
projected/sampled builder gate is section 35, not mesh handoff.

### 35. Probe side-aware station-seam projected PCurve builder

```bash
cd /Volumes/Samsung\ SSD/hpa-mdo/hpa_meshing_package
PYTHONPATH=src /Volumes/Samsung\ SSD/hpa-mdo/.venv/bin/python -m hpa_meshing.cli main-wing-station-seam-side-aware-projected-pcurve-builder-probe \
  --out .tmp/runs/main_wing_station_seam_side_aware_projected_pcurve_builder_probe
```

This report-only probe tries `GeomProjLib.Curve2d`, sampled surface projection
with `Geom2dAPI_Interpolate`, and sampled surface projection with
`Geom2dAPI_PointsToBSpline` on the side-aware candidate station edges. The
current committed result is
`side_aware_station_projected_pcurve_builder_partial`: 36 / 36 operations build
bounded PCurves and pass endpoint orientation, with sampled projection residual
max `1.8343894894033213e-15 m`, but still leave 0 / 12 edge-face pairs passing
the full ShapeAnalysis metadata gate. This is negative-control evidence that
the next repair belongs upstream in section parametrization or export PCurve
metadata generation.

### 36. Probe side-aware station-seam export opcode variants

```bash
cd /Volumes/Samsung\ SSD/hpa-mdo/hpa_meshing_package
PYTHONPATH=src /Volumes/Samsung\ SSD/hpa-mdo/.venv/bin/python -m hpa_meshing.cli main-wing-station-seam-side-aware-export-opcode-variant-probe \
  --out .tmp/runs/main_wing_station_seam_side_aware_export_opcode_variant_probe \
  --materialize-variants
```

This report-only probe generates report-local OpenCSM candidates from the
side-aware profile-parametrization audit. It evaluates simple opcode variants
without changing provider defaults. The current committed result is
`side_aware_export_opcode_variant_not_recovered`: `upper_lower_spline_split`
materializes as `1 volume / 52 surfaces` but still has suspect station BRep /
PCurve metadata, while `all_linseg` materializes as `1 volume / 582 surfaces`
and is stopped by the surface-count guard. This rules out simple opcode
switching as the main-wing product repair; the next gate is direct inspection
of export PCurve metadata generation.

### 37. Audit station-seam export metadata ownership

```bash
cd /Volumes/Samsung\ SSD/hpa-mdo/hpa_meshing_package
PYTHONPATH=src /Volumes/Samsung\ SSD/hpa-mdo/.venv/bin/python -m hpa_meshing.cli main-wing-station-seam-export-metadata-source-audit \
  --out .tmp/runs/main_wing_station_seam_export_metadata_source_audit \
  --external-src-root /Users/linyuan/.local/esp/current/EngSketchPad
```

This report-only audit records the ownership boundary after the opcode variants
fail. Current evidence is
