...
- `main_wing` / `tail_wing` / `fairing_solid` / `fairing_vented` 的 schema、family dispatch、route registry 已經存在；`main_wing` 已有 real ESP/VSP geometry、real Gmsh mesh handoff、real SU2 handoff、solver executed but not converged artifact、reference-geometry warn gate，以及 station-seam BRep / same-parameter / ShapeFix feasibility evidence；目前主翼產品化 blocker 是 station PCurve / seam topology repair，不是 production CFD pass；`tail_wing` 已有 real geometry / surface / blocker probes，`fairing_solid` 已有 real VSP geometry smoke、bounded real mesh handoff probe、real SU2 handoff materialization probe 與 external reference override handoff，但都還不是正式可交付 CFD 路徑。
...
### 23. Write the main wing mesh-handoff smoke
...
### 24. Write the main wing SU2-handoff smoke
...
### 25. Write the tail wing ESP-rebuilt geometry smoke
...
### 26. Write the tail wing mesh-handoff smoke
...
### 27. Write the tail wing SU2-handoff smoke
...
### 28. Probe the real tail wing mesh handoff
...
### 29. Probe the real tail wing surface mesh
...
### 30. Probe naive tail wing solidification
...
### 31. Probe explicit tail wing volume routes
...
2. rebuild the main-wing station PCurves or change the station-seam export strategy around curves 36 and 50 before promoting a compound meshing policy or spending more solver budget
