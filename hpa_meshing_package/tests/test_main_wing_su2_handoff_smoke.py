...
def test_main_wing_su2_handoff_smoke_materializes_without_running_solver(tmp_path: Path):
    ...
    assert report.force_surface_scope == "whole_aircraft_wall"
    ...
    assert "main_wing_component_specific_force_marker_missing" in report.blocking_reasons
...
def test_main_wing_su2_handoff_smoke_writer_outputs_json_and_markdown(tmp_path: Path):
    ...
    assert payload["component_force_ownership_status"] == "missing"
