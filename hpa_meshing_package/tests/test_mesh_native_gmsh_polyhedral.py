...
def test_write_boundary_layer_block_core_tet_mesh_can_preserve_triangulated_interface(
    tmp_path: Path,
):
    pytest.importorskip("gmsh")
    block = build_wing_boundary_layer_block(
    ...
    report = write_boundary_layer_block_core_tet_mesh(
        block,
        farfield,
        tmp_path / "core_preserved_tris.msh",
        su2_path=tmp_path / "core_preserved_tris.su2",
        mesh_size=1.0,
    ...
    assert report["status"] == "meshed"
    assert report["interface_conformality"]["status"] == "preserved"
    assert report["interface_conformality"]["expected_boundary_representation"] == "triangulated"
    assert report["inner_boundary"]["generated_mesh_element_counts"][
        "bl_outer_interface"
    ]["element_type_counts"] == {"2": 20}
    assert report["quality_metrics"]["pyramid_element_count"] == 0
    assert report["quality_metrics"]["non_positive_min_sicn_count"] == 0
    assert report["quality_metrics"]["non_positive_min_sige_count"] == 0
    assert report["quality_metrics"]["non_positive_volume_count"] == 0
    assert report["mesh_quality_gate"]["status"] == "pass"
    assert report["mesh_sizing"]["preserved_boundary_representation"] == "triangulated"
...
def test_write_faceted_volume_mesh_with_boundary_layer_writes_mixed_su2_mesh(
    ...
    assert report["compute"]["mesh_algorithm3d"] == 10
    assert report["mesh_sizing"]["surface_triangulation_policy"] == "shorter_diagonal"
    assert report["physical_groups"]["wing_wall"]["entity_count"] == 24
    assert report["physical_groups"]["farfield"]["entity_count"] == 12
    assert report["mesh_quality_gate"]["status"] == "pass"
...
def test_write_faceted_boundary_layer_su2_case_prepares_no_slip_runtime(
    ...
    assert report["runtime"]["surface_triangulation_policy"] == "shorter_diagonal"
    assert report["mesh_report"]["compute"]["gmsh_threads"] == 4
    assert report["mesh_report"]["compute"]["mesh_algorithm3d"] == 10
    assert report["mesh_report"]["mesh_sizing"]["surface_triangulation_policy"] == "shorter_diagonal"
...
def test_boundary_layer_mesh_quality_gate_blocks_bad_prisms():
    ...
    assert "boundary_layer_severe_low_p01_min_sicn" in gate["blockers"]
    assert "boundary_layer_low_p01_min_sicn" in gate["warnings"]


def test_boundary_layer_mesh_quality_gate_blocks_severely_low_positive_prisms():
    gate = _boundary_layer_mesh_quality_gate(
        core_quality_metrics={
            "tetra_element_count": 10,
            "non_positive_min_sicn_count": 0,
    ...
    assert gate["status"] == "fail"
    assert gate["blockers"] == ["boundary_layer_severe_low_p01_min_sicn"]
    assert gate["warnings"] == ["boundary_layer_low_p01_min_sicn"]
