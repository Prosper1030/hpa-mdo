...
def test_main_wing_route_readiness_includes_side_aware_pcurve_metadata_builder_probe(
    tmp_path: Path,
):
    root = _fixture_report_root(tmp_path)
    _write_json(
    ...
    stage = stages["station_seam_side_aware_pcurve_metadata_builder_probe"]
    assert stage.status == "blocked"
    assert stage.evidence_kind == "real"
    assert (
        stage.observed["metadata_builder_status"]
        == "side_aware_station_pcurve_metadata_builder_partial"
    )
    assert stage.observed["target_edge_count"] == 2
    assert stage.observed["strategy_attempt_summary"]["best_bounded_face_count"] == 4
...
def test_main_wing_route_readiness_records_openvsp_defect_station_audit_stage(
    tmp_path: Path,
):
    root = _fixture_report_root(tmp_path)
    real_mesh_path = (
            ...
            "station_mappings": [
                {
                    ...
                    "nearest_rule_section": {"source_section_index": 3},
                },
                {
                    "defect_station_y_m": 13.5,
                    "nearest_rule_section": {"source_section_index": 4},
                },
            ],
            "engineering_findings": [
    ...
    stages = {stage.stage: stage for stage in report.stages}
    station_stage = stages["openvsp_defect_station_audit"]
    assert station_stage.status == "pass"
    assert station_stage.evidence_kind == "real"
    assert station_stage.observed["alignment_summary"]["defect_station_count"] == 2
    assert station_stage.observed["station_mappings"][0]["defect_station_y_m"] == -10.5
    assert report.next_actions[0] == (
        "trace_defect_edges_to_gmsh_entities_at_openvsp_section_stations"
    )
...
def test_main_wing_route_readiness_records_gmsh_curve_station_rebuild_audit_stage(
    tmp_path: Path,
):
    root = _fixture_report_root(tmp_path)
    real_mesh_path = (
    ...
    _write_json(
        ...
        / "main_wing_gmsh_curve_station_rebuild_audit.v1.json",
        {
            ...
            "curve_matches": [
                {"curve_tag": 36, "relative_length_delta": -0.01},
                {"curve_tag": 50, "relative_length_delta": -0.02},
            ],
            "engineering_findings": [
                "curve_tags_match_vsp3_section_profile_scale"
            ],
    ...
    stages = {stage.stage: stage for stage in report.stages}
    audit_stage = stages["gmsh_curve_station_rebuild_audit"]
    assert audit_stage.status == "pass"
    assert audit_stage.evidence_kind == "real"
    assert audit_stage.observed["match_summary"]["matched_curve_tags"] == [36, 50]
    assert report.next_actions[0] == (
        "build_minimal_openvsp_section_station_topology_fixture"
    )


def test_main_wing_route_readiness_records_openvsp_section_station_topology_fixture_stage(
    tmp_path: Path,
):
    root = _fixture_report_root(tmp_path)
    real_mesh_path = (
    ...
    _write_json(
        ...
        / "main_wing_openvsp_section_station_topology_fixture.v1.json",
        {
            ...
            "engineering_findings": [
                "real_defect_station_topology_fixture_materialized"
            ],
            "next_actions": [
    ...
    stages = {stage.stage: stage for stage in report.stages}
    fixture_stage = stages["openvsp_section_station_topology_fixture"]
    assert fixture_stage.status == "pass"
    assert fixture_stage.evidence_kind == "real"
    assert fixture_stage.observed["fixture_summary"]["candidate_curve_tags"] == [36, 50]
    assert report.next_actions[0] == (
        "decide_station_seam_repair_before_solver_iteration_budget"
    )
...
def test_main_wing_route_readiness_records_station_seam_shape_fix_feasibility_stage(
    tmp_path: Path,
):
    root = _fixture_report_root(tmp_path)
    real_mesh_path = (
    ...
    _write_json(
        ...
        / "main_wing_station_seam_shape_fix_feasibility.v1.json",
        {
            ...
            "engineering_findings": [
                "shape_fix_edge_did_not_recover_station_curve_checks"
            ],
            "blocking_reasons": ["station_shape_fix_repair_not_recovered"],
            "next_actions": [
    ...
    stages = {stage.stage: stage for stage in report.stages}
    shape_fix_stage = stages["station_seam_shape_fix_feasibility"]
    assert shape_fix_stage.status == "blocked"
    assert shape_fix_stage.evidence_kind == "real"
    assert shape_fix_stage.observed["attempt_summary"]["attempt_count"] == 25
    assert "shape_fix_edge_did_not_recover_station_curve_checks" in (
        shape_fix_stage.observed["engineering_findings"]
    )
    assert "station_shape_fix_repair_not_recovered" in report.blocking_reasons
    assert report.next_actions[0] == (
        "rebuild_station_pcurves_or_export_station_seams_before_meshing_policy"
    )
...
def test_main_wing_route_readiness_records_station_seam_export_strategy_probe_stage(
    tmp_path: Path,
):
    root = _fixture_report_root(tmp_path)
    real_mesh_path = (
    ...
    _write_json(
        ...
        / "main_wing_station_seam_export_strategy_probe.v1.json",
        {
            ...
            "engineering_findings": [
                "candidate_materialized_but_needs_station_brep_validation"
            ],
            "blocking_reasons": [
                "candidate_needs_station_brep_pcurve_validation_before_route_promotion"
            ],
            "next_actions": [
    ...
    stages = {stage.stage: stage for stage in report.stages}
    strategy_stage = stages["station_seam_export_strategy_probe"]
    assert strategy_stage.status == "pass"
    assert strategy_stage.evidence_kind == "real"
    assert strategy_stage.observed["target_rule_section_indices"] == [2, 9]
    assert "candidate_materialized_but_needs_station_brep_validation" in (
        strategy_stage.observed["engineering_findings"]
    )
    assert (
        "candidate_needs_station_brep_pcurve_validation_before_route_promotion"
        in report.blocking_reasons
