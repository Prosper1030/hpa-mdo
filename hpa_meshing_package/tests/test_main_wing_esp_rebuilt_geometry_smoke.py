...
def test_main_wing_esp_rebuilt_geometry_smoke_materializes_real_provider_geometry(
    tmp_path: Path,
):
    _skip_without_esp_runtime()

    report = build_main_wing_esp_rebuilt_geometry_smoke_report(tmp_path / "smoke")

    assert report.schema_version == "main_wing_esp_rebuilt_geometry_smoke.v1"
    ...
    assert report.effective_component == "main_wing"
    ...
    assert "esp_rebuilt_main_wing_geometry_materialized" in report.hpa_mdo_guarantees
    assert "main_wing_real_geometry_mesh_handoff_not_run" in report.blocking_reasons


def test_main_wing_esp_rebuilt_geometry_smoke_writer_outputs_json_and_markdown(
    tmp_path: Path,
):
    _skip_without_esp_runtime()

    paths = write_main_wing_esp_rebuilt_geometry_smoke_report(tmp_path / "smoke")
    ...
    assert payload["effective_component"] == "main_wing"
