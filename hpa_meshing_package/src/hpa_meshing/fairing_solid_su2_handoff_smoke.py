...
from .fairing_solid_mesh_handoff_smoke import build_fairing_solid_mesh_handoff_smoke_report
...
class FairingSolidSU2HandoffSmokeReport(BaseModel):
    schema_version: Literal["fairing_solid_su2_handoff_smoke.v1"] = (
    ...
    source_mesh_smoke_schema: Literal["fairing_solid_mesh_handoff_smoke.v1"] = (
...
def build_fairing_solid_su2_handoff_smoke_report(
    out_dir: Path,
) -> FairingSolidSU2HandoffSmokeReport:
    out_dir.mkdir(parents=True, exist_ok=True)
    mesh_smoke_dir = out_dir / "artifacts" / "mesh_handoff"
    try:
        mesh_smoke = build_fairing_solid_mesh_handoff_smoke_report(mesh_smoke_dir)
        mesh_case_report = Path(mesh_smoke.case_dir) / "report.json"
        if mesh_smoke.smoke_status != "mesh_handoff_pass":
            return FairingSolidSU2HandoffSmokeReport(
                ...
                source_mesh_smoke_path=str(
                    mesh_smoke_dir / "fairing_solid_mesh_handoff_smoke.v1.json"
                ),
                source_mesh_case_report_path=str(mesh_case_report),
                blocking_reasons=[
                    "fairing_solid_mesh_handoff_not_available",
                    "su2_handoff_not_materialized",
                ],
                limitations=[
                    "The upstream fairing mesh handoff smoke did not pass.",
                ],
        ...
        return FairingSolidSU2HandoffSmokeReport(
            ...
            component_force_ownership_status="insufficient_evidence",
            blocking_reasons=[
                "fairing_solid_su2_handoff_materialization_failed",
            ],
            limitations=[
...
def _render_markdown(report: FairingSolidSU2HandoffSmokeReport) -> str:
    lines = [
...
def write_fairing_solid_su2_handoff_smoke_report(
    out_dir: Path,
    report: FairingSolidSU2HandoffSmokeReport | None = None,
) -> Dict[str, Path]:
    if report is None:
        report = build_fairing_solid_su2_handoff_smoke_report(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / "fairing_solid_su2_handoff_smoke.v1.json"
    markdown_path = out_dir / "fairing_solid_su2_handoff_smoke.v1.md"
