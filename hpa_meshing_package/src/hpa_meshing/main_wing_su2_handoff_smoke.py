...
from .main_wing_mesh_handoff_smoke import build_main_wing_mesh_handoff_smoke_report
...
class MainWingSU2HandoffSmokeReport(BaseModel):
    schema_version: Literal["main_wing_su2_handoff_smoke.v1"] = (
    ...
    source_mesh_smoke_schema: Literal["main_wing_mesh_handoff_smoke.v1"] = (
...
def build_main_wing_su2_handoff_smoke_report(
    out_dir: Path,
) -> MainWingSU2HandoffSmokeReport:
    out_dir.mkdir(parents=True, exist_ok=True)
    mesh_smoke_dir = out_dir / "artifacts" / "mesh_handoff"
    try:
        mesh_smoke = build_main_wing_mesh_handoff_smoke_report(mesh_smoke_dir)
        mesh_case_report = Path(mesh_smoke.case_dir) / "report.json"
        if mesh_smoke.smoke_status != "mesh_handoff_pass":
            return MainWingSU2HandoffSmokeReport(
                ...
                source_mesh_smoke_path=str(
                    mesh_smoke_dir / "main_wing_mesh_handoff_smoke.v1.json"
                ),
                source_mesh_case_report_path=str(mesh_case_report),
                blocking_reasons=[
                    "main_wing_mesh_handoff_not_available",
                    "su2_handoff_not_materialized",
                ],
                limitations=[
                    "The upstream main-wing mesh handoff smoke did not pass.",
                ],
        ...
        return MainWingSU2HandoffSmokeReport(
            ...
            component_force_ownership_status="insufficient_evidence",
            blocking_reasons=[
                "main_wing_su2_handoff_materialization_failed",
            ],
            limitations=[
    ...
    if component_force_status != "owned":
        blocking_reasons.insert(0, "main_wing_component_specific_force_marker_missing")
    ...
    return MainWingSU2HandoffSmokeReport(
        ...
        source_mesh_smoke_path=str(mesh_smoke_dir / "main_wing_mesh_handoff_smoke.v1.json"),
        source_mesh_case_report_path=str(mesh_case_report),
...
def _render_markdown(report: MainWingSU2HandoffSmokeReport) -> str:
    lines = [
...
def write_main_wing_su2_handoff_smoke_report(
    out_dir: Path,
    report: MainWingSU2HandoffSmokeReport | None = None,
) -> Dict[str, Path]:
    if report is None:
        report = build_main_wing_su2_handoff_smoke_report(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / "main_wing_su2_handoff_smoke.v1.json"
    markdown_path = out_dir / "main_wing_su2_handoff_smoke.v1.md"
