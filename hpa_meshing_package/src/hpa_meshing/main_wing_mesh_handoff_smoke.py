...
class MainWingMeshHandoffSmokeReport(BaseModel):
    schema_version: Literal["main_wing_mesh_handoff_smoke.v1"] = (
    ...
    fixture_kind: Literal["synthetic_occ_thin_wing_closed_solid"] = (
        "synthetic_occ_thin_wing_closed_solid"
    )
...
def _write_thin_wing_step_fixture(out_dir: Path) -> Path:
    ...
    geo_path = fixture_dir / "main_wing_slab.geo"
    step_path = fixture_dir / "main_wing_slab.step"
    geo_path.write_text(
        'SetFactory("OpenCASCADE");\n'
        "Box(1) = {0, -1.0, -0.025, 1.0, 2.0, 0.05};\n",
        encoding="utf-8",
    )
...
def build_main_wing_mesh_handoff_smoke_report(
    out_dir: Path,
) -> MainWingMeshHandoffSmokeReport:
    out_dir.mkdir(parents=True, exist_ok=True)
    try:
        fixture_path = _write_thin_wing_step_fixture(out_dir)
    except Exception as exc:
        return MainWingMeshHandoffSmokeReport(
    ...
    case_dir = out_dir / "artifacts" / "cases" / "main_wing"
    result = run_job(
        MeshJobConfig(
            component="main_wing",
            ...
            metadata={
                "reference_geometry": {
                    "ref_area": 2.0,
                    "ref_length": 1.0,
                    "ref_origin_moment": {"x": 0.5, "y": 0.0, "z": 0.0},
                    "area_method": "synthetic_wing_slab.area",
                    "length_method": "synthetic_wing_slab.chord",
                    "moment_method": "synthetic_wing_slab.centroid",
                    "warnings": [],
                },
            },
        )
    ...
    return MainWingMeshHandoffSmokeReport(
        ...
        hpa_mdo_guarantees=[
            "synthetic_thin_wing_direct_cad_fixture_loaded",
        ...
        limitations=[
            "Synthetic thin closed-solid slab is a route smoke fixture, not aerodynamic wing geometry.",
...
def _render_markdown(report: MainWingMeshHandoffSmokeReport) -> str:
    lines = [
        "# main_wing mesh_handoff smoke v1",
        "",
        "This is a real Gmsh non-BL mesh-handoff smoke for the main-wing route.",
...
def write_main_wing_mesh_handoff_smoke_report(
    out_dir: Path,
    report: MainWingMeshHandoffSmokeReport | None = None,
) -> Dict[str, Path]:
    if report is None:
        report = build_main_wing_mesh_handoff_smoke_report(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / "main_wing_mesh_handoff_smoke.v1.json"
    markdown_path = out_dir / "main_wing_mesh_handoff_smoke.v1.md"
