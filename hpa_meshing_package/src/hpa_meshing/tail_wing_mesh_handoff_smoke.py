...
class TailWingMeshHandoffSmokeReport(BaseModel):
    schema_version: Literal["tail_wing_mesh_handoff_smoke.v1"] = (
    ...
    execution_mode: Literal["real_gmsh_non_bl_mesh_handoff_smoke"] = (
        "real_gmsh_non_bl_mesh_handoff_smoke"
    )
    fixture_kind: Literal["synthetic_occ_thin_tail_closed_solid"] = (
        "synthetic_occ_thin_tail_closed_solid"
    )
...
def _write_thin_tail_step_fixture(out_dir: Path) -> Path:
    ...
    geo_path = fixture_dir / "tail_wing_slab.geo"
    step_path = fixture_dir / "tail_wing_slab.step"
    geo_path.write_text(
        'SetFactory("OpenCASCADE");\n'
        "Box(1) = {0, -0.45, -0.015, 0.55, 0.9, 0.03};\n",
        encoding="utf-8",
    )
    ...
    if completed.returncode != 0 or not step_path.exists():
        message = completed.stderr or completed.stdout or "gmsh STEP export failed"
        raise RuntimeError(message.strip())
    return step_path


def _marker_summary_status(marker_summary: Dict[str, object]) -> MarkerSummaryStatusType:
    tail_wing = marker_summary.get("tail_wing")
    aircraft = marker_summary.get("aircraft")
    farfield = marker_summary.get("farfield")
    if (
        isinstance(tail_wing, dict)
        and tail_wing.get("exists") is True
...
def _wall_marker_status(marker_summary: Dict[str, object]) -> WallMarkerStatusType:
    tail_wing = marker_summary.get("tail_wing")
    if isinstance(tail_wing, dict) and tail_wing.get("exists") is True:
...
def build_tail_wing_mesh_handoff_smoke_report(
    out_dir: Path,
) -> TailWingMeshHandoffSmokeReport:
    out_dir.mkdir(parents=True, exist_ok=True)
    try:
        fixture_path = _write_thin_tail_step_fixture(out_dir)
    except Exception as exc:
        return TailWingMeshHandoffSmokeReport(
    ...
    case_dir = out_dir / "artifacts" / "cases" / "tail_wing"
    result = run_job(
        MeshJobConfig(
            component="tail_wing",
            ...
            metadata={
                "reference_geometry": {
                    ...
                    "area_method": "synthetic_tail_slab.area",
                    "length_method": "synthetic_tail_slab.chord",
                    "moment_method": "synthetic_tail_slab.centroid",
                    "warnings": [],
                },
            },
        )
    ...
    wall_status = _wall_marker_status(marker_summary)
    pass_status = (
    ...
    return TailWingMeshHandoffSmokeReport(
        ...
        wall_marker_status=wall_status,
        ...
        hpa_mdo_guarantees=[
            "synthetic_thin_tail_direct_cad_fixture_loaded",
            "gmsh_thin_sheet_surface_dispatched",
            "mesh_handoff_v1_written",
            "non_bl_runtime_confirmed",
            "tail_wing_wall_and_farfield_markers_present",
            "tail_wing_specific_force_marker_present",
        ]
        if pass_status
        else [
            "synthetic_thin_tail_direct_cad_fixture_loaded",
...
def _render_markdown(report: TailWingMeshHandoffSmokeReport) -> str:
    lines = [
...
def write_tail_wing_mesh_handoff_smoke_report(
    out_dir: Path,
    report: TailWingMeshHandoffSmokeReport | None = None,
) -> Dict[str, Path]:
    if report is None:
        report = build_tail_wing_mesh_handoff_smoke_report(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / "tail_wing_mesh_handoff_smoke.v1.json"
    markdown_path = out_dir / "tail_wing_mesh_handoff_smoke.v1.md"
