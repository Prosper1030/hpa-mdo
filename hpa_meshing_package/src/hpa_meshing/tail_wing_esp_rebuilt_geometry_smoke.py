...
class TailWingESPRebuiltGeometrySmokeReport(BaseModel):
    schema_version: Literal["tail_wing_esp_rebuilt_geometry_smoke.v1"] = (
...
def build_tail_wing_esp_rebuilt_geometry_smoke_report(
    out_dir: Path,
    source_path: Path | None = None,
) -> TailWingESPRebuiltGeometrySmokeReport:
    out_dir.mkdir(parents=True, exist_ok=True)
    source = _default_source_path() if source_path is None else source_path
    fixture_kind: Literal["blackcat_004_origin_vsp3", "custom_vsp3"] = (
        "blackcat_004_origin_vsp3"
        if source.resolve() == _default_source_path().resolve()
        else "custom_vsp3"
    )
    if not source.exists():
        return TailWingESPRebuiltGeometrySmokeReport(
    ...
    try:
        ...
        return TailWingESPRebuiltGeometrySmokeReport(
            ...
            blocking_reasons=[
                "tail_wing_esp_rebuilt_geometry_smoke_failed",
                "tail_real_geometry_mesh_handoff_not_run",
            ],
            limitations=[
    ...
    pass_status = (
        ...
        and selection.get("effective_component") == "horizontal_tail"
    ...
    return TailWingESPRebuiltGeometrySmokeReport(
        ...
        provider_status=(
            provider.get("status") if provider.get("status") in {"materialized", "failed"} else "unavailable"
        ),
        validation_status=(
        ...
        hpa_mdo_guarantees=[
            ...
            "tail_wing_selected_as_horizontal_tail",
...
def _render_markdown(report: TailWingESPRebuiltGeometrySmokeReport) -> str:
    lines = [
...
def write_tail_wing_esp_rebuilt_geometry_smoke_report(
    out_dir: Path,
    report: TailWingESPRebuiltGeometrySmokeReport | None = None,
    source_path: Path | None = None,
) -> Dict[str, Path]:
    if report is None:
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / "tail_wing_esp_rebuilt_geometry_smoke.v1.json"
    markdown_path = out_dir / "tail_wing_esp_rebuilt_geometry_smoke.v1.md"
