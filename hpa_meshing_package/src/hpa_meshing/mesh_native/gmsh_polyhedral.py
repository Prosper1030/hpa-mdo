...
def _configure_gmsh_threads(gmsh: Any, gmsh_threads: int) -> dict[str, int]:
    """Configure Gmsh's internal thread budget and record the applied options."""
    thread_count = max(1, int(gmsh_threads))
    for option in (
        "General.NumThreads",
        ...
        gmsh.option.setNumber(option, thread_count)
    return {
        "gmsh_threads": thread_count,
        "general_num_threads": int(gmsh.option.getNumber("General.NumThreads")),
        "mesh_max_num_threads_1d": int(gmsh.option.getNumber("Mesh.MaxNumThreads1D")),
        "mesh_max_num_threads_2d": int(gmsh.option.getNumber("Mesh.MaxNumThreads2D")),
        "mesh_max_num_threads_3d": int(gmsh.option.getNumber("Mesh.MaxNumThreads3D")),
    }
...
def write_faceted_volume_mesh_with_boundary_layer(
    ...
    *,
    ...
    The wing and farfield surfaces are still generated by hpa-mdo. Gmsh is used
    to extrude the already-owned wall facets into topological prism layers and
    to tetrahedralize only the core outside the BL interface.
    """
    import gmsh
    ...
    if resolved_farfield_mesh_size <= 0.0:
        raise ValueError("farfield_mesh_size must be positive")
    if boundary_layer_first_height <= 0.0:
        raise ValueError("boundary_layer_first_height must be positive")
    if boundary_layer_growth_ratio <= 1.0:
    ...
    try:
        ...
        volume_element_count = sum(volume_type_counts.values())
        bl_type_counts = _entity_type_counts(gmsh, 3, bl_volume_tags)
        ...
        return {
            "status": "meshed",
            "route": "mesh_native_faceted_gmsh_boundary_layer_volume",
            "mesh_path": str(msh_path),
            "su2_path": None if su2_output_path is None else str(su2_output_path),
            "volume_count": len(gmsh.model.getEntities(3)),
            "node_count": len(node_tags),
            ...
            "caveats": [
                "wing boundary is faceted from mesh-native panels",
                "boundary-layer prisms are generated by Gmsh topological extrusion from mesh-native facets",
                "Gmsh topological boundary layers are simple extrusion; sharp tip/TE quality must be checked",
            ],
        }
    finally:
        gmsh.finalize()
...
def _layer_cumulative_heights(
    ...
    heights: list[float] = []
    total = 0.0
    current = float(first_height)
    for _ in range(int(layers)):
        total += current
        heights.append(float(total))
        current *= float(growth_ratio)
    return heights


def _entity_element_count(gmsh, dim: int, entity_tags: Sequence[int]) -> int:
    return sum(_entity_type_counts(gmsh, dim, entity_tags).values())


def _entity_type_counts(gmsh, dim: int, entity_tags: Sequence[int]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for entity_tag in entity_tags:
        ...
        for element_type, tags in zip(element_types, element_tags):
            key = str(int(element_type))
            counts[key] = counts.get(key, 0) + len(tags)
    return dict(sorted(counts.items(), key=lambda item: int(item[0])))


def _collect_entity_quality_metrics(
    gmsh,
    dim: int,
    ...
    for entity_tag in entity_tags:
        _, tags_by_type, _ = gmsh.model.mesh.getElements(dim, int(entity_tag))
    ...
    min_sicn = _element_quality_values(gmsh, element_tags, "minSICN")
    min_sige = _element_quality_values(gmsh, element_tags, "minSIGE")
    volume = _element_quality_values(gmsh, element_tags, "volume")
    return {
...
def _element_quality_values(gmsh, element_tags: Sequence[int], measure: str) -> list[float]:
    try:
        return [float(value) for value in gmsh.model.mesh.getElementQualities(element_tags, measure)]
    except Exception:
        return []
